...
class Wiki(PlugApp):
    ...
    def setUI(self):
        ...
        self.search.list.returnPressed.connect(
                self.on_searchListConfirm)
        self.search.input.returnPressed.connect(
                self.on_searchInputConfirm)

        self.network=Network()
        self.network.returnPressed.connect(self.open)
        self.network.inputTextChanged.connect(
                self.on_networkInputTextChanged)
        self.network.browser.loadCSS(self.css_path)
        # self.network.browser.page().setBackgroundColor(Qt.white)

        self.mindmap=Mindmap()
        self.mindmap.inputTextChanged.connect(
                self.on_mindmapInputTextChanged)
