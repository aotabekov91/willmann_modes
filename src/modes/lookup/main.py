...
class LookupMode(PlugApp):
    ...
    def translate(self, term=None, lan='en'): 

        if not self.ui.isVisible(): self.ui.show()

        if not term: 
            term=self.ui.main.input.text()
        else:
            self.ui.main.input.setText(term)

        self.notes=self.lookupWord(term, lan)
        ...
        if self.notes:
            ...
            dlist=[{'up':'The word is not found', 'down': term}]
            self.ui.main.setList(dlist)
